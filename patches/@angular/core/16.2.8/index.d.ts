...
/**
 * Encapsulated `ProviderWithScope`s that are only accepted during creation of an `EnvironmentInjector` (e.g.
 * in an `NgModule`).
 *
 ...
 *
 * This wrapper type prevents access to the `ProviderWithScope`s inside.
 *
...
/**
 * Creates a token that can be used in a DI ProviderWithScope.
 *
...
/**
 * Wrap an array of `ProviderWithScope`s into `EnvironmentProviders`, preventing them from being accidentally
